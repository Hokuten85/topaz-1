...
ability_object.onUseAbility = function(player, target, ability, action)
    ...
    if math.random() <= math.max(0.8,getHitRate(player, target, true, player:getMod(xi.mod.STEP_ACCURACY))) then
        hit = 6
        ...
        if (mjob == 19) then
            if (target:hasStatusEffect(xi.effect.SLUGGISH_DAZE_1)) then
                ...
                if (player:hasStatusEffect(xi.effect.PRESTO)) then
                    target:addStatusEffect(xi.effect.SLUGGISH_DAZE_3, 1, 0, duration+45)
                    daze = 3
                    effect = 3
                else
                    target:addStatusEffect(xi.effect.SLUGGISH_DAZE_2, 1, 0, duration+45)
                    daze = 2
                    effect = 2
                end
            ...
            elseif (target:hasStatusEffect(xi.effect.SLUGGISH_DAZE_2)) then
                ...
                if (player:hasStatusEffect(xi.effect.PRESTO)) then
                    target:addStatusEffect(xi.effect.SLUGGISH_DAZE_4, 1, 0, duration+45)
                    daze = 3
                    effect = 4
                else
                    target:addStatusEffect(xi.effect.SLUGGISH_DAZE_3, 1, 0, duration+45)
            ...
            elseif (target:hasStatusEffect(xi.effect.SLUGGISH_DAZE_3)) then
                ...
                if (player:hasStatusEffect(xi.effect.PRESTO)) then
                    target:addStatusEffect(xi.effect.SLUGGISH_DAZE_5, 1, 0, duration+45)
                    daze = 3
                    effect = 5
                else
                    target:addStatusEffect(xi.effect.SLUGGISH_DAZE_4, 1, 0, duration+45)
